...
Car::Car(int _year, const char* _model, Colors _color, Brands _brand): Year(_year), Color(_color), Brand(_brand) { // constructor with formal arguments 
		Model = new char[30];
		strcpy(Model, _model);
...
Car::Car(const Car& avto): Year(avto.Year), Model(new char[30]), Color(avto.Color), Brand(avto.Brand) { // overloaded deep copy constructor
