...
Group::Group(Person leader, std::vector<Person> def_group, size_t def_size, std::string name) : Leader(leader), group(def_group), size(def_size), its_my_name(name) {}
...
std::ostream& operator<<(std::ostream& os, Group& my_Group) {
	...
	for (int i = 0; i < my_Group.size; ++i) {
		os << i + 1 << ' ' << my_Group.group[i].Get_Name() << ' ' << my_Group.group[i].Get_Birth() << ' ' << my_Group.group[i].Get_SecondName() << '\n';
	}
	return os;
}
