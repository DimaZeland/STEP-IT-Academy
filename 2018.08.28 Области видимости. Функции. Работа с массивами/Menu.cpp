...
Menu::ConverterWeight(void)const { // weight converter
	...
	while (flag) {
		system("cls");
		cout << "Enter <value>, <type weight[kg, gramm, tonna, centner]>, <type converter[kg, gramm, tonna, centner]>: ";
...
Menu::ProgramInterface() const { // program interface
	...
	do {
		cout << "Please select an action (1 - calculator, 2 - currency converter, 3 - converter weight, 4 - trip cost calculation, 0 - exit):";
		cin >> action;
		system("cls");
		switch (action) {
		case 1:
