...
XOclass::WinCheck(void) { // checking the field of the game for the victory of one of the participants
	...
	for (int i = 1; i < 4; ++i) // checking the game field for having a victory horizontally and vertically
		if (fieldGame[i][1] != ' ' && fieldGame[i][1] == fieldGame[i][2] && fieldGame[i][2] == fieldGame[i][3] && fieldGame[i][3] == fieldGame[i][1]) {
			fieldGame[i][1] == 'X' ? Winner = Player1 : Winner = Player2;
			return true;
		}
		else if (fieldGame[1][i] != ' ' && fieldGame[1][i] == fieldGame[2][i] && fieldGame[2][i] == fieldGame[3][i] && fieldGame[3][i] == fieldGame[1][i]) {
			fieldGame[1][i] == 'X' ? Winner = Player1 : Winner = Player2;
			return true;
		}

		// check the game field for a victory on the diagonals
		if (fieldGame[1][1] != ' ' && fieldGame[1][1] == fieldGame[2][2] && fieldGame[2][2] == fieldGame[3][3] && fieldGame[3][3] == fieldGame[1][1]) {
			fieldGame[1][1] == 'X' ? Winner = Player1 : Winner = Player2;
			return true;
		}
		else if (fieldGame[3][1] != ' ' && fieldGame[3][1] == fieldGame[2][2] && fieldGame[2][2] == fieldGame[1][3] && fieldGame[1][3] == fieldGame[3][1]) {
