...
void Array::RandompArr(
        bool method = 0) { // filling the array with random numbers (method == true) or manually (method == false)
    int *value = new int(0);
...
void Array::Find2Elements() const { // Output max and before the max element of each line
    ...
    for (int i = 0; i < 5; ++i)
        for (int k = 0; k < 5; ++k) {
            if (pArr[i][k] >=
                arr[i][0]) { // The condition for finding the maximum value taking into account the displacement of the previous maximum by the maximum value
                *buffer = arr[i][0];
                arr[i][0] = pArr[i][k];
                if (*buffer > arr[i][1])
                    arr[i][1] = *buffer;
...
void Array::FindGrowStroka() const { // search for the number of rows where all numbers go in ascending order
    ...
    cout << "\nIn an array pArr[5][5] " << *score
         << " line, where the values of all the elements go in ascending order\n";
    delete score;
    score = NULL;
    return;
}
    ...
    for (int k = 0; k < 5; ++k)
        for (int i = this->Get_size() - 1; i > 0; --i) {
            ...
            if (i == 1) {
                *score += ' ';
                *score += (k +
                           48); // in the string, the values are written with symbols on the indexes of the ANSI table,
                //so the symbol of the number 0 has an index of 48, 1, - 49, and so on
            }
        }
    score->size() > 1 ? cout << "\nIn an array pArr[5][5] the elements of the following columns go down: " << *score
                             << endl
...
void Array::MatrixSide() {// changing the values of the sides
    ...
    for (int i = 0; i < 5; ++i) // search of a line
        for (int first = 0, last = this->Get_size() - 1;
             first < last; ++first, --last) // enumeration of values of elements of one line
