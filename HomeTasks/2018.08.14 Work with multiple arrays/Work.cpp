...
void
Work::FindDownElements(const ushort &itsFirst,
                       const ushort &itsSecond) const { // search for all rows whose elements go down
    ...
    if (arr1.empty())
        cout << "\nIn an array arr[" << itsFirst << "][" << itsSecond
             << "] there are no rows in which all the elements go down.\n";
    else {
        cout << "\nIn an array arr[" << itsFirst << "][" << itsSecond
             << "] on the decline all the elements of the following lines: ";
...
ushort
Work::FindLastPairLine(const ushort &itsFirst,
                       const ushort &itsSecond) const { // return the last line number only with paired elements
    // Preconditions: passing function arguments as references to save memory
...
void
Work::SetValue(const ushort &itsFirst,
               const ushort &itsSecond) { // determining the values of array elements arr [][] by random numbers from 0 to 9
    // Preconditions: passing function arguments as references to save memory
...
void
Work::ShowValue(const ushort &itsFirst,
                const ushort &itsSecond) const { // output of array elements arr [][] to the console
    // Preconditions: passing function arguments as references to save memory
    for (int i = 0; i < itsFirst; ++i) {
        for (int j = 0; j < itsSecond; ++j)
            cout << arr[i][j] << ' ';
