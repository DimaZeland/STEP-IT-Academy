...
int main() {
	...
	//7
	int size = std::count_if(arr.begin(), arr.end(), [=](int a) {if (a > 99 || a < -99) return true; else return false; });
	std::vector<int> v3(size);
	std::remove_copy_if(arr.begin(), arr.end(), v3.begin(), [=](auto a) {if (a < 100 && a > -100) return true; else return false; });
	std::cout << "\nvector v3:\n";
	for (auto& x : v3)
		std::cout << x << '\n';
	//8
	...
	std::for_each(arr.begin(), arr.end(), [&](int a) {if (0 == a % 5 && 0 != a) { std::cout << i << '\n'; ++j; }++i; });
	//10
	std::vector<int> v4(j);
	std::copy_if(arr.begin(), arr.end(), v4.begin(), [=](int a) {if (0 == a % 5 && 0 != a) return true; else return false; });
	std::cout << "\nvector v4:\n";
	for (auto& x : v4)
		std::cout << x << '\n';
