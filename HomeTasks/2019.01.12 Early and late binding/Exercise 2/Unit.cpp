...
Unit::Attack(Unit *defender) {
    if (Alive() & defender->Alive())// if the attacker and defender are still alive
        if (defender->Avoid_Attacks()) { // successful attack bias
            ...
            std::cout << "\nAttack succeeded! The " << typeid(*defender).name() + 5 << " has " << defender->hp
                      << " hp.\n";
            if (!defender->hp) //
...
operator<<(std::ostream &os, const Unit *pUnit) {
    os << typeid(*pUnit).name() + 5 << " hp: " << pUnit->hp << ", dmg: " << pUnit->dmg << ", dodge: " << pUnit->dodge
       << std::endl;
    return os;
}
...
Team::Team(std::string Name, unsigned int members) : name(Name) {
    for (; members > 0; --members) // adding players to the team
        switch (rand() % 3) {
            case 0:
                comando.push_back(new Swordsman);
                break;
            case 1:
                comando.push_back(new Archer);
                break;
            case 2:
                comando.push_back(new Mage);
                break;
            default:
                comando.push_back(new Mage);
                break;
...
Game::GameCourse() {
    ...
    while (Good_boys->TeamAlive() & Bad_boys->TeamAlive()) { // play till complete victory
        ...
        std::cout << "\nNow attacking " << typeid(*attacker).name() + 5 << " (" << attack_team->name
                  << " team),\ndefending " <<
