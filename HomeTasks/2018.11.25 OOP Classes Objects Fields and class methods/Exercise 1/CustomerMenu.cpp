...
CustomerMenu::Deposit_replenishment() {
	...
	do
	{
		std::cout << "\nAccount balance " << Clients[its_my_current_account - 1].its_my_deposit << ' ' << Clients[its_my_current_account - 1].its_my_currency_name << '\n';
		std::cout << "\nPlease enter the refill amount <" << Clients[its_my_current_account - 1].its_my_currency_name << "> or press <0> to exit: ";
		std::cin >> sum;
		system("cls");
		if (0 == sum) { // exit
			return false;
			...
			std::cout << "\nThe deposit replenishment was successful. Account balance " << Clients[its_my_current_account - 1].its_my_deposit <<
...
CustomerMenu::Show_current_account_info() {
	std::cout << "\nCurrent account is " << Clients[its_my_current_account - 1].its_my_account_number << '\n' <<
		"Deposit: " << Clients[its_my_current_account - 1].its_my_deposit << ' ' << Clients[its_my_current_account - 1].its_my_currency_name << '\n';
}
	...
	if (Clients.size() > 1) {
		...
		for (BankAccount x : Clients)
			if (x.its_my_account_number != Clients[its_my_current_account - 1].its_my_account_number) {
				std::cout << std::setw(10) << x.its_my_account_number << std::setw(10) << x.its_my_currency_name << std::setw(10) << x.its_my_deposit << '\n';
			}
		return true;
	}
	else {
...
CustomerMenu::Transfer_between_accounts() {
	...
	std::cout << "\nMaximum available for transfer amount: " << Clients[its_my_current_account - 1].its_my_deposit <<
		' ' << Clients[its_my_current_account - 1].its_my_currency_name << '\n';
	std::cout << "Please enter the amount of funds to transfer<" << Clients[its_my_current_account - 1].its_my_currency_name << ">:";
	double sum = 0;
	std::cin >> sum;
	system("cls");
	...
	sum = Money_conversion(sum, Clients[its_my_current_account - 1].its_my_currency, Clients[refunding - 1].its_my_currency); // convert sum to currency recipient
...
CustomerMenu::Withdraw_money() {
	...
	if (0 > its_my_current_account) {
		...
		do
		{
			std::cout << "\nAvailable for removal " << Clients[its_my_current_account - 1].its_my_deposit << ' ' << Clients[its_my_current_account - 1].its_my_currency_name << '\n';
			std::cout << "\nPlease enter the amount to withdraw <" << Clients[its_my_current_account - 1].its_my_currency_name << "> or press <0> to exit: ";
			std::cin >> sum;
			system("cls");
			if (0 == sum) { // exit
				return false;
				...
				Clients[its_my_current_account - 1].Set_deposit(-sum);
				std::cout << "\nThe withdrawal operation was successful. Account balance " << Clients[its_my_current_account - 1].its_my_deposit << ' ' <<
					Clients[its_my_current_account - 1].its_my_currency_name << '\n';
				return true;
			}
